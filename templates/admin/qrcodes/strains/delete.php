...
<div class="af-admin-content__title-row">
    <div class="af-admin-content__title"><?= e(mb_strtoupper(t('admin.strains.delete_page_title'))) ?></div>
...
<div class="af-divider af-mb-4" style="max-width:260px;margin:0 0 18px 0">
    <span class="af-gold af-fs-9 af-track-3"><?= e(t('admin.strains.delete_kicker')) ?></span>
</div>
...
<div class="af-panel af-mb-4" style="max-width:560px;border-color:var(--af-gold)">
    <div class="af-editorial af-w-500 af-mb-3 af-gold" style="font-size:22px;line-height:1.2">
        ⚠ <?= e(t('admin.strains.delete_title')) ?>
    </div>
    <p class="af-fs-12 af-soft" style="line-height:1.7">
        <?= t('admin.strains.delete_body', ['title' => '<strong>' . e($qr->title) . '</strong>']) ?>
    </p>
...
<form method="post" action="/admin/strains/<?= e((string) $qr->id) ?>/delete" style="max-width:560px">
    <?= Session::csrfField() ?>
    <div class="af-flex af-gap-3">
        <button type="submit" class="af-btn af-btn--sm" style="background:var(--af-gold);color:var(--af-bg);border-color:var(--af-gold)">
            <?= e(mb_strtoupper(t('admin.strains.delete_btn'))) ?>
        </button>
...
$bodyContent = $content;
$title = t('admin.strains.delete_page_title');
