<?php
/**
 * Confirmação de exclusão permanente (hard-delete) de uma nota.
 * Admin only. Requer digitação do título exato (similar ao delete user).
 *
 * Vars esperadas:
 *   $qr           → QrCode (já validado: existe + admin + is_deleted=1)
 *   $scanCount    → int
 *   $errors       → list<string>
 */
...
<div class="af-admin-content__title-row">
    <div class="af-admin-content__title"><?= e(mb_strtoupper(t('admin.notes.hard_delete_page_title'))) ?></div>
...
<div class="af-divider af-mb-4" style="max-width:260px;margin:0 0 18px 0">
    <span class="af-blood af-fs-9 af-track-3"><?= e(t('admin.notes.hard_delete_kicker')) ?></span>
</div>
...
<div class="af-panel af-mb-4" style="max-width:560px;border-color:var(--af-blood)">
    <div class="af-editorial af-w-500 af-mb-3 af-blood" style="font-size:22px;line-height:1.2">
        ⚠ <?= e(t('admin.notes.hard_delete_title')) ?>
    </div>

    <p class="af-fs-12 af-soft" style="line-height:1.7">
        <?= t('admin.notes.hard_delete_body', [
...
<form method="post" action="/admin/notes/<?= e((string) $qr->id) ?>/delete-hard" style="max-width:560px">
    <?= Session::csrfField() ?>

    <div class="af-label af-mb-2" style="line-height:1.6">
        <?= t('admin.notes.hard_delete_confirm_label', [
            'title' => '<span class="af-mono af-phosphor" style="text-transform:none">'
                       . e($qr->title) . '</span>',
        ]) /* HTML */ ?>
    </div>
...
$bodyContent = $content;
$title = t('admin.notes.hard_delete_page_title');
